          ...
          <section class="py-3 bg-gray-100" style="height: 200px;">
           ...
           <div class="section" style="width: 800px; margin: auto">
              ...
              <div class="card mt-5">
                  ...
                  <button class="btn insertBtn" onclick="location.href='story_write.do'" >글쓰기</button>
                 </c:if>
              </div>
              ...
              <div class="card mt-5" id="search">
                ...
                <select class="form-select" style="width: 140px" >
                  ...
                  <select name="searchType" id="searchType" class="form-select" style="width: 120px">
                      ...
                      <option value="con"<c:out value="${cri.searchType eq 'con' ? 'selected' : '' }"/>>내용</option>
                  </select>
                  <input class="form-control" type="text" placeholder="검색" style="width: 443.03px" 
              ...
              <div class="card mt-5">
                    ...
                    <table class="table table-bordered table-hover mb-0" >
                      ...
                      <tbody class="border-top-0">
                          ...
                          <tr >
                             <td style="width: 50px">${board.ps_article_id }</td>
                             <td style="width: 80px"> ${board.ps_head }</td>
                            <td id="title" style="width: 780px; height: 20px;">
                            	<a href="story_view.do?ps_article_id=${board.ps_article_id }&page=${cri.page}&perPageNum=${cri.perPageNum}&searchType=${cri.searchType}&keyword=${cri.keyword}"> 
                            	${board.ps_title }
                            	<c:if test="${board.ps_commCnt > 0}">
									<small>(${board.ps_commCnt})</small>
								</c:if>
                            	</a>
							</td>
                            <td id="writer" style="width: 100px">${board.user_id }</td>
                            <td style="width: 80px"><fmt:formatDate value="${board.ps_date }" pattern="yyyy.MM.dd"/></td>
                            <td style="width: 50px">${board.ps_count }</td>
                         </tr>
						</c:forEach>
                     </tbody>
                  </table>  
           ...
           <div class="ms-lg-auto">
              <ul class="pagination m-0">
			     <c:if test="${pagemaker.prev }">
					...
					href="story_list.do?${pagemaker.makeSearch(pagemaker.startPage -1)}">«</a></li>
 				 </c:if>
 				 
              	<c:forEach begin="${pagemaker.startPage}" end="${pagemaker.endPage}" var="pageNum">
                  <li class="page-item"><a class="page-link" 
                  href="story_list.do${pagemaker.makeSearch(pageNum)}">${pageNum }</a></li>
             	<!-- 클래스명에  active 달기-->
             	</c:forEach>
             	...
             	<c:if test="${pagemaker.next && pagemaker.endPage >0 }">
	                <li class="page-item">
	                   <a class="page-link" 
	                   href="story_list.do${pagemaker.makeSearch(pagemaker.endPage+1)}">»</a>
	                </li>
   				</c:if>
              </ul>
          	</div>
