...
$(function(){
	...
	var hytotalPostCnt = $('#hyPostCnt').val(); // 포스트 수
		...
		$.ajax({
			...
			success: function(data){
				...
				$(data).each(function(){
					...
					// 로그인 했을 경우만 좋아요 확인 함
					if( $('#logUser_id').val() != "" ){ // footer에 적혀있음
	...
	$('#hpostDiv').on('click', "img[id^='hylikeYBtn']", function(){
		...
		if( $('#logUser_id').val() == "" ){ // 로그인 아이디 footer에 적혀있음
			alert("로그인 후 이용해 주세요");
			return;
		};
		...
		$.ajax({
			...
			data: {
				...
				'user_id' : $('#logUser_id').val() // 로그인 아이디 footer에 적혀있음
			},
			success: function(data){
				console.log(data.hp_like);
...
<body>
	<!-- hidden 포스트 갯수 -->
	<input id="hyPostCnt" type="hidden" value="${hyPostCnt}">
	<!-- 검색에 저장 -->
					...
					<div class="col-lg-12 my-2" style="width: 100%">
						<ol class="breadcrumb dark-link m-0 small justify-content-center"
							...
							<li class="breadcrumb-item"><a class="text-nowrap"
								href="5_hyangsta_all.do" style="color: #e63946;"><i
					...
					<div class="row gy-4">
						...
						<c:if test="${not empty sessionScope.user_id}">
							<div class="col-12 pt-3 text-end">
								<!-- Button -->
								<button id="hp_wirte" class="btn btn-primary" onclick="location.href='5_hyangsta_writePost.do'">
									포스트쓰기
								</button>
							</div>
						</c:if>
						<!-- 검색  -->
						<form action="5_hyangsta_all.do" method="post"
							class="col-12 mb-3 input-group" style="width: 100%">
							<!-- 셀렉트 -->
							...
							<!-- Search button -->
							<button id="serarchBtn" class="btn btn-primary shadow-none"
								type="submit">
								<i class="bi bi-search"></i>
							</button>
						</form>
						<div id="hpostDiv" class="row gy-4">
						<!-- 향스타 포스트 들어갈 부분 -->
						</div>
					</div>
				</div>
